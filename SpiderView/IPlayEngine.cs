...
namespace Spider.Media
{
    ...
    public class PlayQueue : ResourceCollection<Track>
    {
        ...
        public PlayQueue(IMusicService service)
            : base(service, new List<Track>())
        {
            this.Queue = new Queue<Track>();
            this.History = new Stack<Track>();
        }
        public PlayQueue(IMusicService service, Context context, List<Track> tracks)
            : base(service, tracks)
        {
